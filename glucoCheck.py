...
#==================================================================================================================
    ...
    def insertNa(self, data):
        for subjectId, data in data.groupby('subjectId'):       
            new = data
            subid = subjectId
            new['GV'] = new.iloc[:,2]
            ...
            for x in x_vals:
                if x in l:
                    ...
                    subid_l.append(subid)
                    counter+=1
                else:
                    y_vals.append(float("NaN"))
                    subid_l.append(subid)
            df = {'subjectId':subid_l, 'Display Time':x_vals, 'GlucoseValue':y_vals}
            df = pd.DataFrame(df)
            gap = df['GlucoseValue'].isna().sum()
            return df, gap
    ...
    def impute(self, dataWithMissing):
        ...
        dataWithMissing = self.insertNa(dataWithMissing)[0]
        gap = self.insertNa(dataWithMissing)[1]
...
#==================================================================================================================
    ...
    def bgri(self, x, units):
        ...
        if (units == 'mg'):
            ...
            return 0
        fBG.replace([np.inf, -np.inf], np.nan, inplace=True)

        rBG = 10 * fBG ** 2 # called BG risk function
        rBG.replace([np.inf, -np.inf], np.nan, inplace=True)
